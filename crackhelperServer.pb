...
Procedure.s genPOWaddress(rb$,re$)
  Protected *rb_l, *re_l, res$, a$, randomkey, pu$, pc$
  Protected *cx, *cy, *rndkey
  ...
  *rndkey = *cx+128
  ...
  Debug "Subrange width  :"+Hex(subrangewidth)
  Debug "Random key      :"+Hex(randomkey)
  Debug "Random key range:"+m_gethex32(*rndkey, 32)  
  Debug "Random pubX     :"+m_gethex32(*cx, 32) 
  Debug "Random pubY     :"+m_gethex32(*cy, 32) 
  Debug "Random pub  c.  :"+pc$
...
Procedure initrange()
  ...
  *bufferEnd=AllocateMemory(32)
  ...
  For i = 0 To settings("1")\deviderint-1
    ...
    If Not i%10000
      Sprint(Chr(8)+Chr(8)+Chr(8)+Chr(8)+Chr(8)+Chr(8)+Chr(8)+Chr(8)+Chr(8)+Chr(8)+Chr(8)+Chr(8)+Chr(8)+Chr(8)+Chr(8)+Chr(8)+Chr(8)+Chr(8)+Str(i),#colorWhite)
    EndIf
  Next i
  PrintN("")
...
Procedure validatekeys(key1$,key2$, powaddress$, address$)
  ...
  If key2$<>""
    ...
    If CompareMemoryString(@addr$, @address$, #PB_String_NoCase)<>#PB_String_Equal
      Sprint("[CHECKER] INVALID SOLUTION KEY ["+key1$+"]",#colorMagenta)
...
Procedure AnaliseRequest(reqid)
  Protected answer_f$, metod_json_answer$, id_json_answer, pars_res, name$, pass$, tempjson, get_work, get_resfalse.s, hash$, Values, currange, totalN, randomcurrange, powaddress$, key1$, key2$
